...
public class RestMemberController {
	...
	@ResponseBody
	public String insertMemmber(@RequestBody String json) { 
			//수신되는 json데이터를 받습니다. :@RequestBody
		ObjectMapper objmapper = new ObjectMapper();
		Member vo = null;
		...
		try {
			//json데이터를 Member 자바객체로 변환합니다.
			vo = objmapper.readValue(json, Member.class);
			mapper.addMember(vo);	//db에 저장
			json_result = objmapper.writeValueAsString(result); //선택적으로 실해ㅔㅇ
		} catch (JsonProcessingException e) {
			e.printStackTrace();
		}

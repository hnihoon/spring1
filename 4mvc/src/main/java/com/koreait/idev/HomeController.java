 ...
 */
@Controller		//@Component 대신에 역활을 표시하는 어노테이션
public class HomeController {
	...
	public void search(String name, int age) {	//메소드의 매개변수는 파라미터를 전달받게 됩니다.
		...
		// int값은 파리미터는 기본이 String null일때 , int변환못하고 오류
	...
	@RequestMapping(value="map")	//method속성은 기본값 RequestMapping.GET
	...
	//여기부터는 controller -> view(또는 새로운 요청의 파라미터)로 데이터를 전달
...
//결론 : model은 redirect에서는 get 요청 파라미터값
		//실행했을때 url을 확인합니다. : http://localhost:8085/idev/?idx=99
	}
	...
	public void modelAttr(@ModelAttribute(value="idx")int idx) {
		...
		//idx는 파라미터 이면서 modelAttr.jsp 로 전달하는 model객체
	}
